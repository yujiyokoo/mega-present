...
def output_header_file( param )
  ...
  file.puts "struct RBuiltinClass mrbc_class_#{param[:class]} = {"
  file.puts "  .sym_id = MRBC_SYM(#{param[:class]}),"
  file.puts "  .num_builtin_method = sizeof(method_symbols_#{param[:class]}) / sizeof(mrbc_sym),"
  file.puts "#if defined(MRBC_DEBUG)"
  file.puts "  .names = \"#{param[:class]}\","
  file.puts "#endif"
  file.puts "  .super = #{param[:super]},"
